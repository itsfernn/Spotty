...
impl TopTracksModel {
    ...
    pub fn load_initial(&self) {
        ...
        let query = BatchQuery {
            source: SongsSource::TopTracks,
            batch: Batch::first_of_size(50),
        };
        self.dispatcher.dispatch_async(Box::pin(async move {
            loader
                .query(query, |_s, song_batch| {
                    BrowserAction::SetTopTracks(Box::new(song_batch)).into()
                })
                .await
        }));
    }
    ...
    pub fn load_more(&self) -> Option<()> {
        ...
        self.dispatcher.dispatch_async(Box::pin(async move {
            loader
                .query(query, |_s, song_batch| {
                    BrowserAction::AppendTopTracks(Box::new(song_batch)).into()
                })
                .await
        }));
        Some(())
...
impl PlaylistModel for TopTracksModel {
    ...
    fn play_song_at(&self, pos: usize, id: &str) {
        let source = SongsSource::TopTracks;
    ...
    fn enable_selection(&self) -> bool {
        self.dispatcher
            .dispatch(AppAction::EnableSelection(SelectionContext::TopTracks));
        true
    }
